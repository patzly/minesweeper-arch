...
case class Field private (matrix: CellMatrix) extends FieldInterface {
  ...
  override def dimension: (Int, Int) =
    if matrix.isEmpty then (0, 0) else (matrix(0).size, matrix.size)

  private def withCellRevealed(
      x: Int,
      y: Int,
  ...
  override def withToggledFlag(x: Int, y: Int): Try[Field] = {
    Try(
      matrix.updated(y, matrix(y).updated(x, matrix(y)(x).asFlagToggled))
    ) match {
  ...
  private def revealRec(
    ...
    val reveal_area = new_matrix.zipWithIndex
      .map((row, y) =>
        row.zipWithIndex.map((col, x) => (col, x, y)).slice(xPos - 1, xPos + 2)
      )
      .slice(yPos - 1, yPos + 2)

    // filter out the cells that are already revealed or are bombs
    val filtered_area = reveal_area.flatten.filter((c, x, y) =>
      !new_revealed.contains((x, y)) && !c.isBomb
    )
