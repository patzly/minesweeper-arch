...
class FileIOSpec extends AnyWordSpec {
  private val testGameState = GameState(
    0,
    3,
    Field(Vector(Vector(Cell(true, true)))),
    1.0,
    1,
    1,
    false,
    List(Field(Vector(Vector(Cell(false, true))))),
    List(Field(Vector(Vector(Cell(true, true)))))
  )
  private val emptyGameState = GameState(0, 0, Field(Vector.empty), 0, 0, 0)
  private val emptyGameState2 =
    GameState(0, 0, Field(Vector(Vector.empty)), 0, 0, 0)
