...
class BaseController @Inject() (
    val factory: FieldFactory,
    fileIO: FileIOInterface
) extends Observable[Event]
    with ControllerInterface {
  private[baseController] var gameState: GameState =
    GameState(0, 0, factory.createField(0, 0, 0), 0, 0, 0)
  private[baseController] var state: BaseControllerState =
    FirstMoveBaseControllerState(this)

  private[baseController] def changeState(
      newState: BaseControllerState
  ): Unit = {
    state = newState
  }
  ...
  override def setup(): Unit = {
    ...
    gameState = gameState.copy(field =
      factory.createField(
        gameState.width,
        gameState.height,
  ...
  override def startGame(
    ...
    gameState = GameState(
      maxUndos,
      maxUndos,
      factory.createField(width, height, bomb_chance),
      bomb_chance,
      width,
      height
    )
