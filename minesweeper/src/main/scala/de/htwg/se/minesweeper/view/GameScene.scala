...
case class GameScene(controller: ControllerInterface) extends Scene {
    ...
    for (iy <- 0 until gridHeight) {
      grid.add(
        new Button {
          styleClass = Seq("cell")
          onMouseClicked = e =>
            if !controller.getGameState.field.getCell(ix, iy).get.isRevealed
            then
              e.getButton match {
          ...
          prefWidth <== Bindings.min(
            grid.widthProperty().divide(gridWidth.doubleValue),
            grid.heightProperty().divide(gridHeight.doubleValue)
          )
          prefHeight <== Bindings.min(
            grid.widthProperty().divide(gridWidth.doubleValue),
            grid.heightProperty().divide(gridHeight.doubleValue)
          )
        },
            ...
            new Button("neustarten") {
              ...
              onMouseClicked = _ => {
                ...
                controller.startGame(
                  width,
                  height,
                  controller.getGameState.bombChance,
                  controller.getGameState.maxUndos
                )
        ...
        new Button("Speicherstand laden") {
          onMouseClicked = e => {
            Gui.openFileDialog(scene.get.getWindow) match {
              case None         => ()
              case Some(selectedFile) => controller.loadGame(selectedFile)
            }
          }
        },
        new Button("Spielstand speichern") {
          onMouseClicked = e => {
            Gui.saveFileDialog(scene.get.getWindow) match {
              case None         => ()
              case Some(selectedFile) => controller.saveGame(selectedFile)
            }
          }
        }
      )
  ...
  def update(field: FieldInterface): Unit = {
    ...
    grid.getChildren.forEach(button => {
      val cell = field
        .getCell(
          JGridPane.getColumnIndex(button),
          JGridPane.getRowIndex(button)
        )
        .get
      ...
      if cell.isFlagged || (cell.isRevealed && !cell.isBomb) then
      ...
      if cell.isFlagged then button.getStyleClass.add("flagged")
        ...
        else if cell.nearbyBombs != 0 then
          button.getStyleClass.add("n" + cell.nearbyBombs.toString)
