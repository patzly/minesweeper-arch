  ...
  "A Field" when {
    "it has 0 rows and columns" should {
      ...
      "fail on countNearbyMines" in {
        zeroField
          .countNearbyMines(0, 0) shouldBe a[Failure[IndexOutOfBoundsException]]
      }
      "fail on withRevealed" in {
        zeroField
          .withRevealed(0, 0) shouldBe a[Failure[IndexOutOfBoundsException]]
      }
    }
    ...
    "it has 3 rows and columns and is empty" should {
      val fieldRevealed =
        Field(Vector.tabulate(3, 3)((x, y) => Cell(true, false)))
      ...
      val fieldHidden =
        Field(Vector.tabulate(3, 3)((x, y) => Cell(false, false)))
    ...
    "it has 3 ros and colums and bombs on the diagonal" should {
      val fieldWithBombs =
        Field(Vector.tabulate(3, 3)((x, y) => Cell(false, x == y)))
