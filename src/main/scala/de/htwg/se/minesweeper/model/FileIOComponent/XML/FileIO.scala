...
class FileIO extends FileIOInterface {
  ...
  private def fieldFromXML(node: scala.xml.Node): FieldInterface = {
    ...
    val rows = matrix \ "row"
    val cells =
      rows.map(row => (row \ "cell").map(cellFromXML).toVector).toVector
  ...
  override def save(gameState: GameState, path: String): Try[Unit] = Try {
    if FileExtension.get(path) != "xml" then
      throw new IllegalArgumentException("File extension must be .xml")
