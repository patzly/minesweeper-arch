...
class FileIO extends FileIOInterface {
  ...
  def load(path: String): Try[GameState] = FileExtension.get(path) match {
    ...
    case _ =>
      Failure(
        new Exception("Unknown file extension, can only load .json or .xml")
      )
  }

  def save(state: GameState, path: String): Try[Unit] =
    FileExtension.get(path) match {
      case "xml"  => xmlIO.save(state, path)
      case "json" => jsonIO.save(state, path)
      case _ =>
        Failure(
          new Exception("Unknown file extension, can only save .json or .xml")
        )
    }
}
